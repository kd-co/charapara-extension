...
// Dom elements
var charaParaTextContainer = document.getElementById('charapara-text-container');
var smallText = document.getElementById('smallText');
var mediumText = document.getElementById('mediumText');
var largeText = document.getElementById('largeText');
var refreshBtn = document.getElementById('refreshBtn');
var copyCtrlSuccess = document.getElementById('copy-control-btn-success-icon');
var copyCtrlText = document.getElementById('copy-control-text');
var activeParaControl;
...
function generateCharapara(paragraph_count){
	...
	charaParaTextContainer.innerHTML = temp_markup;
	charaParaTextContainer.classList.add('fadeIn');
}

function handleParagraphChange(){
	activeParaControl = this;
	charaParaTextContainer.classList.remove('fadeIn');
	smallText.classList.remove('control-btn-active');
	mediumText.classList.remove('control-btn-active');
	largeText.classList.remove('control-btn-active');
	activeParaControl.classList.add('control-btn-active');
	setTimeout(function(){
		generateCharapara(parseInt(activeParaControl.dataset.para));
		copyToClipboard();
	}, 100);
}
...
function copyToClipboard() {
  ...
  textarea.value = document.getElementById('charapara-text-container').innerText
  var selector = document.querySelector('#temp_element')
  selector.select()
  document.execCommand('copy')
  document.body.removeChild(textarea);

	copyCtrlSuccess.classList.remove('hide');
	copyCtrlText.innerHTML = 'അടിച്ചു മാറ്റി!';
  setTimeout(function(){
		copyCtrlSuccess.classList.add('hide');
		copyCtrlText.innerHTML = 'അടിച്ചു മാറ്റുക';
  }, 900);
}

function randomizeCharapara(){
	handleParagraphChange.call(activeParaControl)
}

handleParagraphChange.call(smallText)

// event listners
// For paragraph selector
var paragraphControlButtons = document.getElementsByClassName("para-control-btn");
for (var i = 0; i < paragraphControlButtons.length; i++) {
    paragraphControlButtons[i].addEventListener('click', handleParagraphChange, false);
}
// For randomizing paragraph
refreshBtn.addEventListener('click', randomizeCharapara, false);
// For copy button click
copyControlBtn.addEventListener('click', copyToClipboard, false);
